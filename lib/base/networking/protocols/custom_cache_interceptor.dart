...
class CustomCacheInterceptor implements Interceptor {
  ...
  Future<void> onRequest(RequestOptions options, RequestInterceptorHandler handler) async {
    ...
    if (connectivityResult != ConnectivityResult.none && !options.path.contains("tumCard")) {
      ...
      final cache = await cacheStore.get(key);
      if (cache != null && DateTime.now().difference(cache.responseDate).inDays <= 30) {
        return handler.resolve(cache.toResponse(options, fromNetwork: false));
      } else {
        cacheStore.delete(key);
      }
  ...
  Future<void> onResponse(Response response, ResponseInterceptorHandler handler) async {
    ...
    final cacheResponse = await CacheResponse.fromResponse(key: key, options: cacheOptions, response: response);
    await cacheStore.set(await cacheResponse.writeContent(cacheOptions, response: response));
